...
function calcularEnvio(tarifaEnvio){
    ...
    if (peso >= pesoVolumetrico){
        return resultado = peso * parseFloat(tarifaEnvio)
    } else if (peso < pesoVolumetrico){
        return resultado = pesoVolumetrico * tarifaEnvio
    } else{
        alert("Hay alguna medida que no corresonde a un valor númerico. Reintente")
    }
}
...
if (asegurar == "Y" || asegurar == "y"){   
    ...
    alert("El valor del envío con seguro es USD " + nuevoPrecio + "\n(Monto sin seguro: USD " + resultado + ")")
